...
pub async fn check_list_reward() {
    let company = tb_companies
        .load::<Companies>(&mut CONN.get().unwrap().get().expect("cant connect database"));
    for i in company.iter() {
        for c in i.iter() {
            let result_query: Result<Vec<SpinRewards>, _> = tb_spin_rewards
                .filter(reward_status.eq("active"))
                .filter(
                    crate::adapters::spi::cfg::schema::tb_spin_rewards::companies_code
                        .eq(&c.companies_code),
                )
                .load::<SpinRewards>(
                    &mut CONN.get().unwrap().get().expect("cant connect database"),
                );
            let entity = result_query
                .ok()
                .unwrap()
                .into_iter()
            ...
            let value = entity
                .into_iter()
                .map(ListRewardPresenterMapper::to_api)
                .collect::<Vec<ListRewardsPresenter>>();
