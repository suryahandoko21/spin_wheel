...
use crate::{adapters::api::{spin_tickets::spin_tickets_payloads::SpinTicketPayload, shared::{app_state::AppState, response::GenericResponse, error_presenter::ErrorReponse}}, application::usecases::{spin_tickets::post_one_spin_tickets::PostSpinTicketUseCase, interfaces::AbstractUseCase}, domain::error::ApiError};
...
async fn post_spin_tickets(data: web::Data<AppState>,post:Json<SpinTicketPayload>) ->Result<HttpResponse,ErrorReponse>{
    ...
    let spin_prizes: Result<GenericResponse, ApiError> = post_one_spin_ticket.execute().await;
    spin_prizes
