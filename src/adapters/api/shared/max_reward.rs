...
use super::{response::ErrorResponse, init_global::GLOBAL_INIT};
...
pub fn max_reward_active_add(payload :&SpinRewardPayload)->(StatusCode,bool,ErrorResponse){
    ...
    if filtered_list.len() > *max_list_active_reward as usize{
        error_msg.message =format!("{} {}", "Max reward active cant more than".to_string(), max_list_active_reward); 
...
pub fn max_reward_active_update(payload :&SpinRewardUpdatedPayload)->(StatusCode,bool,ErrorResponse){
    let mut error_msg = ErrorResponse{
        message: "".to_string(),
        status: "".to_string()
    };
