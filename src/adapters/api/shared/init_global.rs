...
pub fn set_global_init() {
    ...
    let token_validation_be =
        dotenv::var("TOKEN_VALIDATION_BE").expect("TOKEN_VALIDATION_BE must be set");
    let enable_token_validation =
        dotenv::var("ENABLE_TOKEN_VALIDATION").expect("ENABLE_TOKEN_VALIDATION must be set");
    let max_list_active_reward =
        dotenv::var("MAX_LIST_ACTIVE_REWARD").expect("MAX_LIST_ACTIVE_REWARD must be set");
    let _ = GLOBAL_INIT.set(
        [
            ...
            (
                "enable_token_validation".to_string(),
