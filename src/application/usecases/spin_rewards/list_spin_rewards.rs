use crate::{
    adapters::api::spin_reward::query_string::QstringReward,
    application::{
    ...
    domain::{error::ApiError, spin_reward_entity::SpinRewardEntity},
};
...
impl<'a> AbstractUseCase<Vec<SpinRewardEntity>> for ListSpinRewardsUseCase<'a> {
    async fn execute(&self) -> Result<Vec<SpinRewardEntity>, ApiError> {
        let spin_rewards = self
            .repository
            .get_all_spin_reward_by_company_code(self.company_code.to_string(), self.qstring)
            .await;
        match spin_rewards {
            Ok(facts) => Ok(facts),
