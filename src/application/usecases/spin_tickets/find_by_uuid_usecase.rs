...
use crate::{
    ...
    domain::{spin_prizes_entity::SpinPrizesEntity, error::ApiError}, adapters::api::shared::response::SpinAvailableResponse,
};
...
impl<'a>AbstractUseCase<SpinAvailableResponse> for GetSpinTicketByUuidUseCase<'a>{
    async fn execute(&self)->Result<SpinAvailableResponse,ApiError>{
        let spin_ticket = self.repository.get_spin_ticket_by_uuid((*self.uuid.clone()).to_string()).await;
        match spin_ticket {
            Ok(facts) => Ok(facts),
