...
use crate::{
    adapters::api::shared::response::SpinAvailableResponse,
    application::{
...
impl<'a> GetSpinTicketByUseridUseCase<'a> {
    pub fn new(userid: &'a String, repository: &'a dyn SpinTicketEntityAbstract) -> Self {
...
impl<'a> AbstractUseCase<SpinAvailableResponse> for GetSpinTicketByUseridUseCase<'a> {
    async fn execute(&self) -> Result<SpinAvailableResponse, ApiError> {
        let spin_ticket = self
            .repository
            .get_spin_ticket_by_uuid((*self.userid.to_string()).to_string())
            .await;
        match spin_ticket {
            Ok(facts) => Ok(facts),
